  ...
  return (
    <div>
      <header className='flex py-2 px-6 sm:px-6 justify-between place-items-center'>
          ...
          {/* Chỉ hiển thị link Create Event khi user có role là admin hoặc organizer */}
          ...
          <Link to={'/wallet'}>
            ...
            {/* Thêm liên kết Admin - Chỉ hiển thị nếu user.role === 'admin' */}
          ...
          {!!user &&(
            <div className="absolute z-10 mt-64 flex flex-col w-48 bg-white right-2 md:right-[160px] rounded-lg shadow-lg"> 
              <nav className={`block ${isMenuOpen ? 'block' : 'hidden'} `}>
                ...
                {/* Chỉ hiển thị Create Event trong menu dropdown cho admin và organizer */}
                {user && (user.role === 'admin' || user.role === 'organizer') && (
                ...
                {/* Thêm liên kết Admin vào menu dropdown - Chỉ hiển thị nếu user.role === 'admin' */}
                {user && user.role === 'admin' && (
